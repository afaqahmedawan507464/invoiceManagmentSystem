            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="row">
                        ...
                        <div class="col-6">
                            <h3 class="text-end">Bank Statement Details Reports</h3>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12 ps-lg-2 mb-3">
                            <div>
                              <div class="card-header">
                                <form action="{{ route('getDataForMonthWiseBankReport') }}" method="post" class="d-flex justify-content-end align-items-center">
                                  @csrf
                                  <div>
                                    <select class="form-control" name="searchByMonthYear">
                                      ...
                                      @if(isset($expense))
                                          @foreach ($expense as $selectExpenses)
                                              @php
                                                  $currentMonth = \Carbon\Carbon::parse($selectExpenses->payment_date)->format('M,Y');
                                              @endphp
                                              @if($currentMonth != $prevMonth)
                                                  <option value="{{ $selectExpenses->payment_date }}">{{ $currentMonth }}</option>
                                              @endif
                                              @php
       ...
       var expense ={!! $selectExpense !!}
        ...
        @else
            @foreach ($selectExpense  as $selectExpenses)
                expenseDate.push({{ \Carbon\Carbon::parse($selectExpenses->payment_date)->format('d') }});
        ...
        @else
            @foreach ($selectExpense as $selectExpenses)
                usePayment.push({{ $selectExpenses->total_send_amount }});
                receivedPayment.push({{ $selectExpenses->total_received_amount }});
            @endforeach
        @endif
        const maxAmount = Math.max(...expense.map(expense => expense.total_received_amount));
        const minAmount = Math.min(...expense.map(expense => expense.total_received_amount));
        const xValues = expenseDate;
        const yValues = usePayment;
        const receivedAmount = receivedPayment;
         new Chart("myChart", {
           type: "bar",
           data: {
             labels: xValues,
